...
| Connectivity beacons | Captive-portal detection off by default: `captive_portal_mode=0` (IGNORE, the only probe-free value) seeded on first boot and retried until set; no probes, no portal auto-open, no network blacklisting | lenos-netd first-boot seed + panel toggle | 1 is the stock PROMPT (probes to Google) and 2 is AVOID (disconnect+blacklist) — the panel writes only 0 and 1; seed failures are visible in the kernel log and retried every cycle |

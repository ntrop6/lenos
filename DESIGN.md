...
| Connectivity beacons | Captive-portal detection off by default: `captive_portal_mode=0` (IGNORE, the only probe-free value) written by the lenOS panel on first open (Settings holds WRITE_SECURE_SETTINGS); no probes, no portal auto-open, no network blacklisting | panel first-open seed + toggle | 1 is the stock PROMPT (probes to Google) and 2 is AVOID (disconnect+blacklist) — the panel writes only 0 and 1, and reads unset as the real default (1) so it can never display a lie |
| Radio downgrade attacks | LTE-only radio toggle (2G/3G code paths dropped, LTE/5G kept — partial GrapheneOS LTE-only parity: GOS additionally drops 5G) | patch 0014 (TelephonyManager, MODIFY_PHONE_STATE) | opt-in switch; the unrestricted mask is saved before enabling and restored on disable |
